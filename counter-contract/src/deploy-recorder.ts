/**
 * Standalone deploy script for the Board Resolution Recorder contract.
 *
 * Usage:
 *   npm run build && npm run deploy:recorder
 *
 * Prompts for a hex seed (or generates a new one), builds a wallet,
 * waits for sync + funds + dust, deploys the recorder contract,
...
import { Contract, ledger as recorderLedger } from './managed/recorder/contract/index.js';
...
const zkConfigPath = path.resolve(currentDir, '..', 'src', 'managed', 'recorder');
...
// Types
// ---------------------------------------------------------------------------
type RecorderPrivateState = { yesVotes: bigint; noVotes: bigint };
type RecorderCircuits = ImpureCircuitId<Contract<RecorderPrivateState>>;
const RecorderPrivateStateId = 'recorderPrivateState' as const;
...
async function main() {
  ...
  console.log('\n╔══════════════════════════════════════════════════════════╗');
  console.log('║      Board Resolution Recorder - Deploy Script          ║');
  console.log(`║      Network: ${NETWORK_ID.padEnd(42)}║`);
  console.log('╚══════════════════════════════════════════════════════════╝\n');
  ...
  const zkConfigProvider = new NodeZkConfigProvider<RecorderCircuits>(zkConfigPath);

  const providers = {
    privateStateProvider: levelPrivateStateProvider<typeof RecorderPrivateStateId>({
      privateStateStoreName: 'recorder-private-state',
  ...
  // --- Compile contract ---
  const recorderCompiledContract = CompiledContract.make('recorder', Contract).pipe(
    CompiledContract.withVacantWitnesses,
    CompiledContract.withCompiledFileAssets(zkConfigPath),
  );

  // --- Deploy ---
  const contract = await withStatus('Deploying Board Resolution Recorder contract (this may take a few minutes)', async () => {
    return deployContract(providers, {
      compiledContract: recorderCompiledContract,
      privateStateId: RecorderPrivateStateId,
      initialPrivateState: { yesVotes: 0n, noVotes: 0n },
    });
  });
  ...
  console.log(`\n╔══════════════════════════════════════════════════════════╗`);
  console.log(`║  Board Resolution Recorder deployed successfully!       ║`);
  console.log(`╚══════════════════════════════════════════════════════════╝`);
